...
public class ClasspathSuite extends Suite {
	...
	private static final boolean DEFAULT_INCLUDE_MANIFEST = false;
	...
	private static ClassesFinder createFinder(Class<?> suiteClass, ClassesFinderFactory finderFactory) {
		return finderFactory.create(getSearchInJars(suiteClass), getParseManifest(suiteClass), getClassnameFilters(suiteClass), getSuiteTypes(suiteClass),
	...
	private static boolean getParseManifest(Class<?> suiteClass) {
		IncludeManifest includeManifestAnnotation = suiteClass.getAnnotation(IncludeManifest.class);
		if (includeManifestAnnotation == null) {
			return DEFAULT_INCLUDE_MANIFEST;
		}
		return includeManifestAnnotation.value();
	}
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.TYPE)
	public @interface IncludeManifest {
		boolean value();
	}
}
